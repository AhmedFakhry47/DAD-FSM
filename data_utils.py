...
class DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        batch_frames_512 = np.zeros((self._time_step+self._num_pred, 3, 64, 64))
        batch_frames = np.zeros((self._time_step+self._num_pred, 3, self._resize_width, self._resize_height))

        for i in range(self._time_step + self._num_pred):
            image_512, h, w = np_load_frame(self.video_frames[frame_index + i], 64, 64)
...
class DADFSM_loader(DataLoader):
    ...
    def __getitem__(self, index):
        ...
        batch_seg_512 = np.zeros((self._time_step+self._num_pred, 3, 64, 64))
        ...
        for i in range(self._time_step + self._num_pred):
            ...
            seg_512, h, w = np_load_frame(self.segmented_frames[seg_index + i], 64, 64,background_sub=False) #Changes made here
...
class DADFSM_trainloader(DataLoader):
    ...
    def seg_setup(self):
        ...
        if os.path.isdir(labels[0]):
            all_label_info = []
            for label in labels:
                label_info = glob.glob(os.path.join(label, '*.txt'))
                label_info.sort(key=lambda x: int(os.path.basename(x).split('.')[0].split('_')[0])) #Change this back from 1 -> 0
    ...
    def __getitem__(self, index):
        ...
        batch_labels = np.zeros((self._time_step + self._num_pred, 3, 64, 64))
        ...
        for i in range(self._time_step + self._num_pred):
            ...
            seg_512, h, w = np_load_frame(self.segmented_frames[seg_index + i], 64, 64)
            seg, h, w = np_load_frame(self.segmented_frames[seg_index + i], self._resize_height,
                                  self._resize_width)

            label=load_label(self.label_info[label_index + i],64,64)
