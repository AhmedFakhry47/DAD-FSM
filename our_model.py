...
class decoder(nn.Module):
    def __init__(self):
        ...
        def layer2transpose(nc, intOutput):
            return torch.nn.Sequential(
...
class DADFSM(nn.Module):
    def __init__(self,args):
        ...
        #self.late_attn = SequentialAttentionBlock(self.args.trans_linear_in_dim,2)

        self.decoder = ahdecoder()
    ...
    def forward(self,x,y):
        ...
        #Spatial attention
        x = self.fr_enrich(x)
        # x = self.temp_average(x)
        # x = x.unsqueeze(1)

        y = self.fr_enrich(y)
        # y = self.temp_average(y)
        # y = y.unsqueeze(1)
        ...
        #z = self.late_attn(z)
        #reshape before decoding the information into an image
        #x = x.view(-1,1,self.args.num_frames*self.args.trans_linear_in_dim)
